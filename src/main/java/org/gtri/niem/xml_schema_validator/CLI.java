...
public class CLI {
    ...
    List<File> subjectXMLFiles = new ArrayList<>();
    ...
    public static void main(String[] args) {
        try {
            ...
            Parser parser = new Parser(Parser.SchemaFullCheckingIndicator.SchemaFullChecking, cli.catalogURIs,
                                       cli.schemaLocations);
    ...
    private void processArgs(String[] args) {
        ...
        options.addOption(new Option(null, "catalog", true,
                                     "An XML Catalog document to direct schema assembly"));
        options.addOption(new Option(null, "schema-location", true,
                                     "A schema location string (like @xsi:schemaLocation)"));
        ...
        if(cl.hasOption("help")) {
            ...
            help.printHelp(Integer.MAX_VALUE, "this is command line syntax", "this is a header",
                           options, "this is a footer", true);
            System.exit(0);
        }
        ...
        for(Option option : cl.getOptions()) {
            switch(option.getLongOpt()) {
                case "catalog":
                    catalogURIs.add(asReadableFile(new File(option.getValue()),
                                                   "XML Catalog file").toURI().toString());
                    break;
